...
export const ResumeSummaryCard = ({
  ...
  return (
    <div
      ...
      onClick={onSelect}
      onMouseEnter={() => setIsHovered(true)} // MOUSE FOR SWIPE FOR MOBILE
...
export const TemplateCard = ({ thumbnailImg, isSelected, onSelect }) => {
  return (
    <div className={`group h-auto md:h-[300px lg:h-[320px]flex flex-col bfpwhite border-2 overflow-hidden cursor-pointer transition-all duration-500 hover:scale-105 hover:shadow-lg rounded-3xl
      ${
        isSelected ? 'border-violet-500 shadow-lg shadow-violter-500/20 bg-violer-50' : 'border-gray-200 hover:border-violet-300' 
      }`} onClick={onSelect}>
        {thumbnailImg ? (
          <div className='relative w-full h-full overflow-hidden'>
            <img src={thumbnailImg || '/placeholder.svg'} alt="Template Review" className='w-full h-full object-cover group-hover:scale-199 transition-transform duration-700'/>
            <div className='absolute inset-0 bg-gradient-to-r from-white/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300'/>
            {isSelected && (
          ...
          <div className='w-full h-[200px] flex items-center flex-col justify-center bg-gradient-to-br from-violet-50 via-violet-600 to-fuchsia-50'>
            <div className='w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-2xl items-center justify-center mb-3'>
